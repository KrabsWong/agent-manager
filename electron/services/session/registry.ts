...
export interface SessionService {
  ...
  /** 获取统计信息 */
  getStats(): Promise<{ totalSessions: number; totalMessages: number }> | { totalSessions: number; totalMessages: number };
...
class SessionServiceRegistry {
   ...
   * 检查是否有指定类型的服务
   */
  has(appType: AppType): boolean {
    return this.services.has(appType);
  }
...
// 导出类型
export type { SessionService as SessionServiceInterface };
