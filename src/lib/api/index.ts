...
export const providersApi = {
  getAll: async (appType: AppType): Promise<Provider[]> => {
    const response = await window.electronAPI.invoke('providers:getAll', appType) as ApiResponse<Provider[]>;
    return extractData(response);
  },

  getById: async (id: string, appType: AppType): Promise<Provider | null> => {
    const response = await window.electronAPI.invoke('providers:getById', id, appType) as ApiResponse<Provider | null>;
    return extractData(response);
  },

  create: async (input: CreateProviderInput): Promise<Provider> => {
    const response = await window.electronAPI.invoke('providers:create', input) as ApiResponse<Provider>;
    return extractData(response);
  },
  ...
  update: async (
    ...
    const response = await window.electronAPI.invoke('providers:update', id, appType, input) as ApiResponse<Provider>;
    return extractData(response);
  },

  delete: async (id: string, appType: AppType): Promise<void> => {
    const response = await window.electronAPI.invoke('providers:delete', id, appType) as ApiResponse<void>;
    return extractData(response);
  },

  switch: async (id: string, appType: AppType): Promise<SwitchProviderResult> => {
    const response = await window.electronAPI.invoke('providers:switch', id, appType) as ApiResponse<SwitchProviderResult>;
    return extractData(response);
  },

  reorder: async (appType: AppType, providerIds: string[]): Promise<void> => {
    const response = await window.electronAPI.invoke('providers:reorder', appType, providerIds) as ApiResponse<void>;
    return extractData(response);
  },
};
...
export const mcpApi = {
  ...
  getById: async (id: string): Promise<McpServer | null> => {
    const response = await window.electronAPI.invoke('mcp:getById', id) as ApiResponse<McpServer | null>;
    return extractData(response);
  },

  getByApp: async (appType: AppType): Promise<McpServer[]> => {
    const response = await window.electronAPI.invoke('mcp:getByApp', appType) as ApiResponse<McpServer[]>;
    return extractData(response);
  },

  create: async (input: CreateMcpServerInput): Promise<McpServer> => {
    const response = await window.electronAPI.invoke('mcp:create', input) as ApiResponse<McpServer>;
    return extractData(response);
  },

  update: async (id: string, input: Partial<CreateMcpServerInput>): Promise<McpServer> => {
    const response = await window.electronAPI.invoke('mcp:update', id, input) as ApiResponse<McpServer>;
    return extractData(response);
  },
  ...
  toggleApp: async (id: string, appType: AppType, enabled: boolean): Promise<void> => {
    const response = await window.electronAPI.invoke('mcp:toggleApp', id, appType, enabled) as ApiResponse<void>;
    return extractData(response);
  },
  ...
  openConfigFolder: async (appType: AppType): Promise<void> => {
    const response = await window.electronAPI.invoke('mcp:openConfigFolder', appType) as ApiResponse<void>;
    return extractData(response);
  },
};
...
export const skillsApi = {
  ...
  getInstalled: async (): Promise<Skill[]> => {
    const response = await window.electronAPI.invoke('skills:getInstalled') as ApiResponse<Skill[]>;
    return extractData(response);
  },

  install: async (repoUrl: string, directory?: string): Promise<Skill> => {
    const response = await window.electronAPI.invoke('skills:install', repoUrl, directory) as ApiResponse<Skill>;
    return extractData(response);
  },
  ...
  toggleApp: async (id: string, appType: AppType, enabled: boolean): Promise<void> => {
    const response = await window.electronAPI.invoke('skills:toggleApp', id, appType, enabled) as ApiResponse<void>;
    return extractData(response);
  },

  scanRepo: async (owner: string, name: string): Promise<Skill[]> => {
    const response = await window.electronAPI.invoke('skills:scanRepo', owner, name) as ApiResponse<Skill[]>;
    return extractData(response);
  },

  getRepoInfo: async (owner: string, name: string): Promise<{ owner: string; name: string; url: string; description?: string; stars: number }> => {
    const response = await window.electronAPI.invoke('skills:getRepoInfo', owner, name) as ApiResponse<{ owner: string; name: string; url: string; description?: string; stars: number }>;
    return extractData(response);
  },
